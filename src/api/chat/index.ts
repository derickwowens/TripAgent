...
// ============================================
// S3 PARK DATABASE HANDLERS
// ============================================

async function handleLookupParkDatabase(
  ...
  try {
    // If park_id is provided, get specific park details
    if (input.park_id) {
      ...
      // Park not found in S3, will try fallback below
      s3Failed = true;
    ...
    if (input.query) {
      ...
      // No results in S3, will try fallback below
      s3Failed = true;
  ...
  // FALLBACK: Try NPS API or State Park Service when S3 fails or has no data
  if (s3Failed && facade) {
    ...
    // Try NPS API for national parks
    ...
    // Try State Park Service for state parks
...
async function handleGetParksNearLocation(
...
async function handleGetParkDatabaseStats(): Promise<any> {
  ...
  try {
    ...
    return {
      ...
      source: 'TripAgent S3 Database',
      note: 'This is our authoritative park database with comprehensive information for all listed sites',
    };
